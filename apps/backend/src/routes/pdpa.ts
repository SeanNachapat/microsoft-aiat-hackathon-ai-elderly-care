...
import { getPatientById, patientsStore } from '../mock/patients';
...
router.get('/', (_req, res) => {
  const summary = patientsStore.map((p) => ({
    patientId: p.id,
    patientName: p.name,
...
router.put('/:patientId', (req, res) => {
  ...
  const { consents } = req.body as { consents: { label: string; status: boolean }[] };
