...
const PRIORITY_CONFIG: Record<string, { color: string; emoji: string; label: string }> = {
 ...
 */
export async function sendLineNotification(action: ActionPayload): Promise<boolean> {
  const token = env.LINE_CHANNEL_ACCESS_TOKEN;
  const userId = env.LINE_CAREGIVER_USER_ID;

  if (!token || !userId) {
    logger.warn("⚠️ LINE credentials not configured — skipping notification");
    return false;
  }
            ...
            contents: [
              ...
              // Health context
              {
                ...
                contents: [
                  ...
                  {
                    type: "text",
                    text: `โรค: ${elder.healthConditions.map((h: any) => h.condition).join(", ")}`,
                    size: "xs",
                    color: "#666666",
                    wrap: true
                  },
                  {
                    type: "text",
                    text: `ยา: ${elder.medications.map((m: any) => `${m.name} ${m.dose} (${m.schedule})`).join(", ")}`,
                    size: "xs",
                    color: "#666666",
                    wrap: true
                  },
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.error(`❌ LINE API error (${response.status}): ${errorBody}`);
      return false;
    }

    logger.info(`✅ LINE notification sent: [${priority.label}] ${actionLabel}`);
    return true;
  } catch (error) {
