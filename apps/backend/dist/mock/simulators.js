...
exports.stopSimulator = exports.startSimulator = void 0;
...
const jitter = (current, range, decimals = 0) => {
...
const computeAlertLabel = (spo2, hr) => {
...
const startSimulator = (io, intervalMs) => {
    if (simulatorInterval)
        return; // already running
    logger_1.default.info(`🔬 Mock IoT simulator started — interval: ${intervalMs}ms`);
    simulatorInterval = setInterval(() => {
        patients_1.patientsStore.forEach((patient) => {
            ...
            const updatedPatient = (0, patients_1.updatePatientVitals)(patient.id, {
                hr: newHr,
                spo2: newSpo2,
                bp: newBp,
                temp: newTemp,
...
const stopSimulator = () => {
    if (simulatorInterval) {
        ...
        logger_1.default.info('🛑 Mock IoT simulator stopped');
    }
};
