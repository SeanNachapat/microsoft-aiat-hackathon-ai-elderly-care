...
router.post('/generate', (req, res) => {
    const { patientId, type = 'daily', period = '2026-05-03' } = req.body;
    let patients = patients_1.patientsStore;
    if (patientId) {
        const p = (0, patients_1.getPatientById)(patientId);
        if (!p) {
    ...
    const report = {
        ...
        patients: patients.map((p) => ({
            ...
            medicationAdherence: p.detail.medications.length > 0
                ? Math.round(p.detail.medications.reduce((sum, m) => sum + m.adherence, 0) /
